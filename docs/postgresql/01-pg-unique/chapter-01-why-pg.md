---
doc_id: pg-why-pg
title: PostgreSQL 凭什么值得选
---

# PostgreSQL 凭什么值得选

> 从 MySQL 转过来的人，PG 到底强在哪？什么场景该选 PG？
...
PG 和 MySQL 的设计出发点就不一样——PG 追求功能完备和标准合规，MySQL 追求简单和性能。这个差异渗透到每一个细节里。

**MVCC 实现**是最典型的例子。PG 把旧版本直接存回原表（堆表），读操作永远不阻塞写操作，但代价是需要定期 VACUUM 清理死元组。MySQL 用 Undo Log 存旧版本，空间回收交给后台线程，不需要手动维护，但在高并发长事务下 Undo Log 膨胀是常见问题。

**SQL 标准遵循**也差很多。MySQL 的 GROUP BY 可以不写全聚合字段，PG 必须写全——这看起来是"不灵活"，实际上是帮你避免写出结果不确定的查询。
