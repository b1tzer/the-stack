...
本文回答一个问题：索引建好了，为什么 `EXPLAIN` 里 `key=NULL`？失效场景归为两类——条件无法翻译成 B+ 树的有序区间，以及优化器基于成本主动放弃。文末给出索引提示（Index Hint）作为最后手段。
...
最左前缀原则的具体表现，推导见 [索引设计 §1.2](./chapter-02-index-design.md)。结论：范围列命中多个值后，后续列不再全局有序，无法沿树定位。设计时把范围列放联合索引末位即可规避。
