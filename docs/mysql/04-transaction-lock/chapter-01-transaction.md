...
- **当前读路径**：靠 `Next-Key Lock` 锁住「记录 + 间隙」，阻止别的并发事务往查询范围内插入新行。间隙锁细节见 [锁机制](./chapter-02-lock.md) §2。
