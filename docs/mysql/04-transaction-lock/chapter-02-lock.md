...
这是理解 InnoDB 行锁的第一道关：**行锁加在索引记录上，不是加在数据行上。**

所谓「锁定某一行」，实际是锁定该行对应的**索引记录**。InnoDB 定位一行时，无论走主键还是二级索引，最终都要落到某个索引条目上，锁就加在这个条目上。

这条认知直接推出一个高频踩坑结论：**如果查询没有走任何索引，InnoDB 无法精确定位要锁的记录，只能锁住主键索引上的所有记录——效果等同于锁全表。**

```sql
-- 表 t 有主键 id，但没有 age 索引
BEGIN;
SELECT * FROM t WHERE age = 30 FOR UPDATE;  -- age 无索引 → 全表扫描 → 锁住所有主键记录
-- 此时任何对 t 的写入都会被阻塞，即便写的是毫不相干的行
```

所以「给经常作为查询条件的列建索引」不仅是性能优化，也是**缩小锁范围**的关键手段。锁的范围，取决于查询实际走的是哪条索引。
...
## 5. 为什么需要 Gap Lock：幻读的代价

回忆 [事务与 MVCC](./chapter-01-transaction.md) §11 的结论：快照读靠 Read View 解决幻读，而**当前读**（`FOR UPDATE` / `LOCK IN SHARE MODE` / DML）靠锁解决幻读。
...
- 它只在 `REPEATABLE READ` 及以上才启用（见 §8）。
...
`READ COMMITTED` 下没有 Gap Lock（有例外：外键约束检查等场景仍会短暂加），锁的范围更小，并发更高，但代价是当前读无法防幻读。这就是 [事务与 MVCC](./chapter-01-transaction.md) §13 提到「高并发读场景可考虑 RC」的锁层面的依据——**RC 牺牲幻读防护，换来更小的锁范围和更高的并发。**

## 10. 死锁：锁之间的环形等待

行锁的粒度细，反而更容易触发死锁。经典场景是两个事务以相反顺序更新两行：
...
InnoDB 能**自动检测**死锁（`innodb_deadlock_detect`，默认开启），检测到后主动回滚其中一个事务（通常是持有锁更少、或修改行数更少的那个），让另一个继续。死锁本身不是 bug，是并发下难以完全避免的现象；工程上要做的是**降低死锁频率**，见 §12。
