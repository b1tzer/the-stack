...
[锁机制](./chapter-03-lock.md) 回答了 InnoDB「有哪些锁、怎么加锁」。本文回答的是另一层问题：**你的业务场景，该不该主动去拿这把锁。** 悲观锁与乐观锁给出了两种相反的答案——前者先加锁再操作，后者先操作、提交时再校验冲突。
...
悲观锁假设并发冲突必然发生，于是在读取数据的瞬间就加锁，把冲突挡在操作之前。落地上，悲观锁几乎全部复用 [锁机制](./chapter-03-lock.md) 里的行锁：`FOR UPDATE` 加排他锁，`LOCK IN SHARE MODE` 加共享锁。
...
2. **悲观锁必须走索引**：无索引等于锁全表，见 [锁机制](./chapter-03-lock.md) §1.3。
