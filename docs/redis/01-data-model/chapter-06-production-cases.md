...
4 个案例的根因全部落在前五章的知识点上。先建立映射，再逐个展开：
...
| SDS 惰性释放 + 分配器碎片 | 内存只涨不降（RSS 偏高） | [案例四](#case-4) |
...
## 5. 案例四：内存只涨不降——惰性释放与分配器碎片 {#case-4}
...
4 个案例的共同点：问题不在 Redis 本身，而在「选错了结构」或「误读了机制」。
...
- [Redis 生产环境大 Key 事故复盘：从内存碎片到集群雪崩](https://tsight.io/articles/11462731)（案例四）
