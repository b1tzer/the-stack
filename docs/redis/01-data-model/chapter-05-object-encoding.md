...
为什么同一份 Hash 数据要用两种编码？紧凑编码（listpack）连续存储、无指针开销，但查询需线性扫描，元素一多就慢；hashtable 查询 O(1)，但有桶数组、节点指针等固定开销，元素少时反而浪费。因此 Redis 在元素少时用紧凑编码省内存，元素多时切换 hashtable 保性能——阈值就是这两者的交点。
