...
HTTP 是请求-响应模型，服务端无法主动推数据给客户端。聊天、实时通知、股票行情这些场景，客户端只能不停地问「有新消息吗？」——这就是轮询，代价很大。
...
打开两个浏览器窗口，在一个窗口发送消息，另一个窗口立即收到。这就是 WebSocket 的核心价值：**服务端主动推送，无需客户端轮询**。
