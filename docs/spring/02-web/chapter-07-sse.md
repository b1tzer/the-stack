...
三行核心代码，一个能跑的 SSE 端点。接下来解释它为什么能工作，以及生产环境需要注意什么。
...
**决策依据**：只需要服务端推送 → SSE（更简单、自动重连）。需要双向通信 → WebSocket。两者不互弃，可以在同一个项目中共存。WebSocket 的完整用法参见 [WebSocket 实时通信](/spring/02-web/chapter-06-websocket)。
